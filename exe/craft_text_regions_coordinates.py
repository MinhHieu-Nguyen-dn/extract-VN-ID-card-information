...
def get_text_regions_coordinates(image_path, result_path='result/stage2_craft_text_regions'):
    ...
    # if cuda:
    #     net.load_state_dict(test.copyStateDict(torch.load(os.path.join(weights_path, trained_model))))
    #     net = net.cuda()
    ...
    # download refiner model weight file
    ...
    if refine:
        ...
        # if cuda:
        #     refine_net.load_state_dict(test.copyStateDict(torch.load(refiner_model)))
        #     refine_net = refine_net.cuda()
        #     refine_net = torch.nn.DataParallel(refine_net)
        # else:
        #     refine_net.load_state_dict(test.copyStateDict(torch.load(refiner_model, map_location='cpu')))
